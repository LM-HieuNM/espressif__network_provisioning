...
typedef struct {
    ...
    bool valid;
} authmode_cache_entry_t;

static authmode_cache_entry_t authmode_cache[MAX_AUTHMODE_CACHE_SIZE] = {0};

/* Get cached authmode for SSID */
static bool get_cached_authmode(const char *ssid, wifi_auth_mode_t *authmode)
{
    for (int i = 0; i < MAX_AUTHMODE_CACHE_SIZE; i++) {
        if (authmode_cache[i].valid && 
            strcmp(authmode_cache[i].ssid, ssid) == 0) {
            *authmode = authmode_cache[i].authmode;
            ESP_LOGI(TAG, "Cache hit: authmode %d for '%s'", *authmode, ssid);
            return true;
        }
    }
    return false;
...
/* Store authmode in cache */
static void cache_authmode(const char *ssid, wifi_auth_mode_t authmode)
{
    ...
    if (slot == -1) {
        /* Cache full, use first slot (simple FIFO) */
        slot = 0;
    }
    
    strncpy(authmode_cache[slot].ssid, ssid, sizeof(authmode_cache[slot].ssid) - 1);
    authmode_cache[slot].ssid[sizeof(authmode_cache[slot].ssid) - 1] = '\0';
    authmode_cache[slot].authmode = authmode;
    authmode_cache[slot].valid = true;
}
...
static esp_err_t wifi_apply_config_handler(network_prov_ctx_t **ctx)
{
    ...
    if (get_cached_authmode((const char*)wifi_cfg->sta.ssid, &cached_authmode)) {
        ...
        if (esp_wifi_scan_start(&scan_config, true) == ESP_OK) {
            ...
            ESP_LOGW(TAG, "Scan failed, using default authmode");
        }
    }
                                  ...
                                  network_prov_scan_ctx_t **ctx)
{
    ...
    cache_authmode((const char *)record->ssid, record->authmode);
    return ESP_OK;
}
#endif // CONFIG_NETWORK_PROV_NETWORK_TYPE_WIFI
